"""
Test suite for the hbz-OPUS-specific implementation
of the source metadata mapper.
"""
import pytest
from lzvnrw_converter.oaipmh_converter import OAIPMHMetadataConverter
from lzvnrw_mapper.hbz_opus import HbzOpusMetadataMapper


@pytest.fixture(name="minimal_source_dict_hbz_opus")
def get_minimal_source_dict_hbz_opus():
    """
    Returns a dict from a given minimal XML for testing the hbz-opus mapper.
    """
...
"""<OAI-PMH>
    ...
    <request>https://hbz.opus.hbz-nrw.de/oai</request>
    <GetRecord>
        <record>
            <header>
                <identifier>oai:hbz.opus:x</identifier>
            </header>
            <metadata>
                <oai_dc:dc>
                    ...
                    <dc:identifier>
                        https://hbz.opus.hbz-nrw.de/frontdoor/index/index/docId/xx
                    </dc:identifier>
                    <dc:identifier>
                        https://hbz.opus.hbz-nrw.de/files/xx/xxxxxxxxxxx.pdf
                    </dc:identifier>
                </oai_dc:dc>
            </metadata>
        </record>
...
@pytest.fixture(name="hbz_opus_mapper")
def get_hbz_opus_mapper():
    """Returns a hbz_opus mapper."""
    return HbzOpusMetadataMapper


def test_hbz_opus_map(
    minimal_source_dict_hbz_opus,
    hbz_opus_mapper
):
    """
    Test the hbz_opus mapper.
    From an input dict, assert that the expected outputs are returned.
    """

    assert hbz_opus_mapper().MAPPER_TAG == "Hbz OPUS Metadata Mapper"

    # DC-Creator
    result_dc_creator = hbz_opus_mapper().get_metadata(
        key="DC-Creator",
        source_metadata=minimal_source_dict_hbz_opus
    )
    ...
    # DC-Title
    result_dc_title = hbz_opus_mapper().get_metadata(
        key="DC-Title",
        source_metadata=minimal_source_dict_hbz_opus
    )
    ...
    # DC-Terms-Identifier
    result_dc_terms_id = hbz_opus_mapper().get_metadata(
        "DC-Terms-Identifier",
        minimal_source_dict_hbz_opus
    )
    ...
    result_origin_system_identifier = \
        hbz_opus_mapper().get_metadata(
            "Origin-System-Identifier",
            minimal_source_dict_hbz_opus
        )
    assert result_origin_system_identifier == "oai:hbz.opus"
    ...
    assert result_source_organization == "https://d-nb.info/gnd/2047974-8"
...
def test_get_specversion(hbz_opus_mapper):
    """
    Test the public get-method for _SPECVERSION.
    """
    mapper_specversion = hbz_opus_mapper().get_specversion()
    assert all(isinstance(n, int) for n in mapper_specversion[0:3])
    assert isinstance(mapper_specversion[3], str)


def test_deleted_record(deleted_record_dict, hbz_opus_mapper):
    """
    Ensure the deleted record is mapped as expected without errors.
    """

    result_source_organization = hbz_opus_mapper().get_metadata(
        key="source-organization",
        source_metadata=deleted_record_dict
    )
    result_origin_system_identifier = hbz_opus_mapper().get_metadata(
        key="origin-system-identifier",
        source_metadata=deleted_record_dict
    )
    result_external_identifier = hbz_opus_mapper().get_metadata(
        key="external-identifier",
        source_metadata=deleted_record_dict
    )
    result_transfer_urls = hbz_opus_mapper().get_metadata(
        key="transfer-urls",
        source_metadata=deleted_record_dict
    )
